...
for(i in c(3,1,2)) {
  ...
  if(i==3) {
    text(1, max(mns+sds)-diff(range(mns+sds))*0.1, expression(paste(E,(italic(rho[x])), " = -0.037")), pos = 4)
  } else if(i==1) {
    text(1, max(mns+sds)-diff(range(mns+sds))*0.1, expression(paste(E,(italic(rho[x])), " = 0.000")), pos = 4)
  } else if(i==2) {
    text(1, max(mns+sds)-diff(range(mns+sds))*0.1, expression(paste(E,(italic(rho[x])), " = 0.095")), pos = 4)
  }
