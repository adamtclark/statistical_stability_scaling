...
tmsim2<-symdyn(r, f, d, d_sd, sf=0.1, tmax=24.99, stocht = FALSE, stochd = FALSE, oscillate_dist=TRUE)
x<-tmsim2[,"state"]
tm<-tmsim2[,"time"]
xstar<-sin(tm)*0.2
...
text(24.99, xstar[length(xstar)], expression(paste(italic(N)[A],"*")), pos = 4, xpd=NA, cex=1.5, col="gray50")
text(24.99, xstar2[length(xstar2)], expression(paste(italic(N)[B],"*")), pos = 4, xpd=NA, cex=1.5, col="gray50")
text(24.99, -1.5, expression(paste(italic(N)[C],"*")), pos = 4, xpd=NA, cex=1.5, col="gray50")
